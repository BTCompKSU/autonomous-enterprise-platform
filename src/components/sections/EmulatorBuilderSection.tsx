...
export function EmulatorBuilderSection() {
  return (
    <section id="emulators" className="scroll-mt-32 border-t bg-muted/30 py-20">
      <div className="mx-auto max-w-5xl px-6">
        <SectionHeader
          eyebrow="For Ops & IT"
          title="Every AI emulator ships with guardrails."
        ...
        <div className="mt-10 rounded-3xl border bg-card p-8 shadow-2xl sm:p-10">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-2xl font-bold sm:text-3xl">{AGENT.name}</h3>
                <p className="text-sm text-muted-foreground">
                  {AGENT.workflow} · <span className="text-success">{AGENT.status}</span>
                </p>
              </div>
            </div>
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-4">
            <Metric icon={Zap} label="Avg confidence" value={`${AGENT.confidence}%`} tone="success" />
            <Metric icon={Workflow} label="Volume / week" value={`${AGENT.weeklyVolume.toLocaleString()}`} tone="brand" />
            <Metric icon={Bot} label="Auto-handled" value={`${AGENT.autoHandled.toLocaleString()}`} tone="primary" />
            <Metric icon={Users} label="Routed to human" value={`${AGENT.routedToHuman}`} tone="muted" />
          </div>
          ...
          <div className="mt-6 rounded-2xl border bg-gradient-to-br from-brand/10 via-card to-primary/10 p-6">
            ...
            <div className="relative mt-3 h-3 overflow-hidden rounded-full bg-muted">
              <div className="absolute inset-y-0 left-0 bg-success/40" style={{ width: `${AGENT.threshold}%` }} />
              <div className="absolute top-0 h-full w-0.5 bg-foreground" style={{ left: `${AGENT.threshold}%` }} />
              <div className="absolute top-0 h-full w-1.5 bg-success" style={{ left: `${AGENT.confidence}%` }} />
            </div>
            ...
            <p className="mt-3 text-sm text-foreground/80">
              Anything below <strong>{AGENT.threshold}%</strong> confidence is automatically routed to a human
              reviewer. The current rolling average is{" "}
              <strong className="text-success">{AGENT.confidence}%</strong>.
            </p>
          </div>

          <div className="mt-8 grid gap-6 sm:grid-cols-2">
            <ScopeCard title="In scope" items={AGENT.scope} tone="success" />
            <ScopeCard title="Out of scope" items={AGENT.outOfScope} tone="muted" />
          </div>

          <div className="mt-8 rounded-2xl border bg-background p-6">
            <div className="flex items-center gap-2">
              <ShieldCheck className="h-5 w-5 text-brand" />
              <h4 className="text-base font-semibold">Governance & oversight</h4>
            </div>
          ...
          <div className="mt-6 rounded-xl border border-success/30 bg-success/10 p-4 text-sm text-success-foreground">
            <strong className="text-success">Estimated impact:</strong> {AGENT.hoursSaved} hours recovered per
            week per AP specialist — redirected into vendor relationships and audit support.
          </div>
        </div>

        <BottomCTA
          line="Build your first emulator in a free audit."
...
function ScopeCard({ title, items, tone }: { title: string; items: string[]; tone: "success" | "muted" }) {
  const tones = {
